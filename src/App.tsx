...
function ShelfRow({ books, offset, cardCount, openBook }: { books: Book[]; offset: number; cardCount: (id: string) => number; openBook: (book: Book) => void }) {
  ...
  const bookPress = useRef<{ pointerId: number; startX: number; startY: number; moved: boolean; book: HTMLButtonElement } | null>(null)
  ...
  const onPointerDown = (event: React.PointerEvent<HTMLDivElement>) => { const book = (event.target as HTMLElement).closest<HTMLButtonElement>('.shelf-book'); if (book) { bookPress.current = { pointerId: event.pointerId, startX: event.clientX, startY: event.clientY, moved: false, book }; return }; drag.current = { pointerId: event.pointerId, startX: event.clientX, startScroll: event.currentTarget.scrollLeft }; event.currentTarget.setPointerCapture(event.pointerId) }
  const onPointerMove = (event: React.PointerEvent<HTMLDivElement>) => { const press = bookPress.current; if (press?.pointerId === event.pointerId && Math.hypot(event.clientX - press.startX, event.clientY - press.startY) > 8) press.moved = true; if (drag.current?.pointerId === event.pointerId) event.currentTarget.scrollLeft = drag.current.startScroll - (event.clientX - drag.current.startX) }
  const onPointerEnd = (event: React.PointerEvent<HTMLDivElement>) => { const press = bookPress.current; if (press?.pointerId === event.pointerId) { bookPress.current = null; if (event.pointerType === 'touch' && !press.moved) { event.preventDefault(); press.book.click() }; return }; if (drag.current?.pointerId === event.pointerId) { drag.current = null; event.currentTarget.releasePointerCapture(event.pointerId) } }
